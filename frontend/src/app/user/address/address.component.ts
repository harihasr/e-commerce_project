...
export class AddressComponent implements OnInit {
       ...
       (response) => {
        if(response['success']){
          ...
          for (var index = 0; index < responses.length; index++) {
            const temp = new AddressModel(responses[index]['address_id'], responses[index]['address_line1'], responses[index]['address_line2'], responses[index]['city'], responses[index]['state'], responses[index]['zip_code'], responses[index]['phone']);
  ...
  onEditAddress(form: NgForm){
    let temp = new AddressModel(form.value.address_id, form.value.address_line1, form.value.address_line2, form.value.city, form.value.state, form.value.zipcode, form.value.phone);
      this.userService.editAddress(temp).subscribe(
        (response) => {
        ...
        (error) => {
          console.log(error)
          this.resText = "Something went wrong! Please try again."
        }
